...
def solve_12_coin_fsm(A, B, C):
    ...
    while state != State.DONE:
        ...
        # ---------------- TRY 2 (C1+C2 vs A1+A2) ----------------
            ...
            if event == Event.BALANCED:
                print("C1+C2 are genuine, looking for fake in C3 or C4")
                state = State.TRY3_C3_A3
  #              c_path = "34"
            else:
                leftrel = "heavier" if event == Event.LEFT else "lighter"
                print(f"C1 or C2 is fake and {leftrel}, looking for fake...")
                leftrel = "heavier" if event == Event.LEFT else "lighter"
...
######## chexk above
        ...
        # ---------------- TRY 3: C1 vs A1 ----------------
            ...
            if event == Event.BALANCED:
                ...
                print(f"C1 is genuine → fake is {fake_coin} ({fake_relation})")
            else:
                fake_coin = "C1"
                fake_relation = "heavier" if C[0] > A[0] else "lighter"
                print(f"{fake_coin} is fake ({fake_relation})")

            state = State.DONE

     # ---------------- TRY 3: C3 vs A3 ----------------
        elif state == State.TRY3_C3_A3:
            ...
            if event == Event.BALANCED:
                fake_coin = "C4"
                fake_relation = rightrel if 'rightrel' in locals() else "unknown"
                print(f"C3 is genuine → fake is {fake_coin} ({fake_relation})")
            else:
                fake_coin = "C3"
                fake_relation = "heavier" if C[2] > A[2] else "lighter"
                print(f"{fake_coin} is fake ({fake_relation})")
